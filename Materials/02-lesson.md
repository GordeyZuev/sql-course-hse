...
На следующей паре CTE чаще связывают с `JOIN`. Здесь: `WITH … AS ( SELECT … )` **+** `SELECT … FROM имя` – тот же смысл, что подзапрос во `FROM`, но имя шага в **начале** запроса; так проще, когда шагов несколько.
